...
> **[Deploy to Netlify](https://app.netlify.com/start/deploy?repository=https://github.com/FriskyDevelopments/open-claude-design)** · **[Admin `/admin`](src/app/admin/page.tsx)** · **[Figma → code](#design-system)** · **[X promo thread](docs/X_PROMO.md)** · **[We speak español → README.md](README.md)**
...
```bash
npm run dev # http://localhost:3000 → Add API key (BYOK modal) → Design in codebase
...
Zeabur note: your `zat_…` is expired (`check`). After `zeabur auth login` → `docs/ZEABUR.md` → `mcp.friskydev.com` → `mcp.zeabur.com` canonical.
...
- Clean rebuild — **no Mobbin PNGs** in repo. Reference only, per Mobbin ToS.
